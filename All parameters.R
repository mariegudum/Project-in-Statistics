...
init_guess <- c(0.02,-0.12,0.55,0.4,0.1,70,5,30,1)
lower_bounds <- c(0.02,-0.12,0.55,0.4,0.1,70,5,30,1)
upper_bounds <- c(0.05,-0.08,0.70,0.5,0.3,100,15,50,4)
...
neg_log_likelihood <- function(params) {
  ...
  for (k in 1:N) {
    ...
    Sigmak <- matrix(0, nrow = 3, ncol = 3)
    Sigmak[1, 1] <- beta_term * S_k
    Sigmak[2, 2] <- ((1 - theta1) * lambda10 + theta1 * lambda11) * beta_term
    Sigmak[3, 3] <- lambda10 * beta_term
    diff <- X[k + 1, 1:3] - X[k, 1:3] - dt * Fk
    SigmaSigmaT <- dt * Sigmak %*% t(Sigmak)
    inv_Sigma <- solve(SigmaSigmaT)
    det_val <- det(SigmaSigmaT)
    ll <- ll + t(diff) %*% inv_Sigma[1:3,1:3] %*% diff + log(det_val)
  }
  return(as.numeric(ll))
}

neg_log_likelihood(true_params$beta2)
neg_log_likelihood_vec<-Vectorize(neg_log_likelihood)
beta0_test<-seq(0.55,0.65,by=0.001)
plot(beta0_test,neg_log_likelihood_vec(beta0_test))

neg_log_likelihood_vec(beta0_test)


init_guess <- c(0.03,-0.08,0.55)
lower_bounds <- c(0.03,-0.12,0.55)
upper_bounds <- c(0.05,-0.08,0.70)
